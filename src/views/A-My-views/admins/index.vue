<template>
  <div>
    <div style="width: 100%">
      <el-table :data="adminInfoList" style="width: 100%" height="600">
            ...
            >
              ...
              <template #dropdown>
                <div style="width: 200px; height: 220px">
                  <el-checkbox-group
                    size="small"
                    v-model="currentCheckedAdminRolesList"
                    @change="adminRolesInfoFilter"
                  >
                    <el-checkbox
                      v-for="(item, idx) in adminRolesInfoFilterOption"
                      :key="idx"
                      :label="item.text"
                      :value="item.value"
        ...
        <el-table-column align="right" width="280" fixed="right">
          <template #header>
            <el-input
              v-model="adminListInfoSearch"
              @input="getadminListInfoSearch"
              :clearable="true"
              resize="both"
            ...
            >
              <template #prepend>
                <el-button
                  v-my-roles="ROLES_KEY.CREATE"
                  type="primary"
                  @click="AddAdminAccountBtn"
                  >添加
                </el-button>
              </template>
            </el-input>
          ...
          <template #default="scope">
            ...
            <el-button
              ...
              @click="editAdminPassward(scope.row.username, scope.row.id)"
              type="primary"
              circle
            ...
            <el-button
              ...
              @click="deleteAdminAccount(scope.row.id, scope.row.username)"
              type="danger"
              circle
              style="position: relative; right: 50px"
            >
  ...
  <!-- 添加管理员 dialog -->
  <el-dialog v-if="isShowComponent(ROLES_KEY.CREATE)" v-model="showAddAdminView">
  ...
  <!-- 修改密码dialog -->
  <el-dialog v-if="isShowComponent(ROLES_KEY.UPDATE)" v-model="showEditAdminPassward" width="50%">
    <!-- 确认修改 -->
    <template #footer>
      <el-button @click="modifyAdminPassward" type="primary" size="small"
        >确认</el-button
      >
    </template>
    <el-form
    ...
    >
      ...
      <el-form-item label="密码 " prop="CurrentAdminPassward">
        <el-input
          v-model="updateInfo.CurrentAdminPassward"
          type="password"
          placeholder="请输入新密码"
          show-password
        />
      ...
      <el-form-item label="确认密码 " prop="CurrentAdminPasswardAgain">
        <el-input
          v-model="updateInfo.CurrentAdminPasswardAgain"
          type="password"
          placeholder="请再次输入密码"
        />
      </el-form-item>
  ...
  <!-- 删除管理员dialog -->
  <el-dialog v-if="isShowComponent(ROLES_KEY.DELETE)" v-model="showDeleteAdminAccount" width="50%">
    <span>确认删除管理员{{ deleteAdminInfo.account }}</span>
    <template #footer>
      <el-button
        @click="ComfirmToDeleteAdminAccount"
        type="primary"
        size="small"
        >确认</el-button
      >
...
const getAdminInfoList = async (
  roles = adminRolesInfoFilterOption.value.map((item) => item.value)
) => {
...
getAllRoles().then((res) => {
  ...
  adminRolesInfoFilterOption.value = res.data.map((item) => ({
    value: item.id,
...
const dropdown1 = ref();
const adminRolesInfoFilter = async (value, row) => {
  // console.log(row.roles.id, value);
  // return row.roles.id === value;
  const _1 = currentCheckedAdminRolesList.value.map(
    (i) => adminRolesInfoFilterOption.value.find((j) => j.text === i).value
  );
  getAdminInfoList(_1);
};
//修改密码
...
const editAdminPassward = (username, id) => {
  ...
  updateInfo.CurrentAdminInEditUserName = username;
};
//修改完成，点击确认
