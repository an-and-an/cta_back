    ...
    <el-table :data="workInfo" style="width:100%" height="600">
                ...
                <el-table-column align="right" width="150px" fixed="right">
                    <template #default="scope">
                        <el-button
                        @click="setFinal(scope.row.teamName, scope.row.leader
  , scope.row.teamMember1, scope.row.teamMember2)" style="position:relative;right:50px;" 
                        key="进入决赛" 
                        type="primary" 
                        text bg>
                        进入决赛</el-button
...
function setFinal(teamName,leader,teamMember1,teamMember2) {
  console.log('点击了设置进入决赛--', teamName, leader, teamMember1, teamMember2)
